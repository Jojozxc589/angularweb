...
export class PriceboothComponent {
  ...
  //       payment_proof_url: this.selectedFile.name // ใช้ URL ที่กรอก
  //     };
   ...
   confirmPayment(): void {
    ...
    if (this.selectedFile) {
      ...
      const formData = new FormData(); // สร้าง FormData object
        formData.append('file', this.selectedFile); // เพิ่มไฟล์
        formData.append('booking_id', this.booking.bookingID);
        this.http.post('https://wag7.bowlab.net/angular-webpro/uploadPaymentSlip', formData).subscribe(
          (uploadResponse: any) => {
            
          }
        )

      this.http.post('https://wag7.bowlab.net/angular-webpro/updateBookingStatus10', payload).subscribe(
        (response: any) => {
          this.loading = false; // ซ่อนสถานะโหลด
          if (response.success) {
            if (response.message === 'ชำระแล้ว') {

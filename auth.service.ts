...
export class AuthService {
  private baseApiUrl = 'https://wag7.bowlab.net/angular-webpro';
...
deleteProject(projectId: number): Observable<any> {
  return this.http.delete<any>(`https://wag7.bowlab.net/angular-webpro/deleteProject18/${projectId}`);
}
////ลบบูธ
...
cancelBooking(bookingId: number): Observable<any> {
  ...
  cancelBookings(bookingId: number) {
    return this.http.post<any>(`https://wag7.bowlab.net/angular-webpro/updateBookingDate`, { booking_id: bookingId });
  }
...
updateEventCode(bookingId: number, eventCode: string): Observable<any> {
 ...
 // ฟังก์ชันสำหรับอัปเดตเหตุการณ์
 updateEvent(event_code: string, event_name: string, start_date: string, end_date: string): Observable<any> {
  const payload = {
  ...
  return this.http.post(`${this.baseApiUrl}/updateEvent15`, payload);
}
}